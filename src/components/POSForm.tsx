...
export const POSForm: React.FC<POSFormProps> = ({ 
  ...
  const updatePartWarranty = (partId: string, patch: { warrantyDurationDays?: number; warrantyTerms?: string }) => {
    setUsedParts(prev => prev.map(part => part.partId === partId ? { ...part, ...patch, hasProductWarranty: (patch.warrantyDurationDays ?? part.warrantyDurationDays ?? 0) > 0 } : part));
  };
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black ${
                    ...
                    {loyaltyStats.tier === 'VIP' ? <Crown className="w-4 h-4" /> : <Star className="w-4 h-4 fill-current" />}
                  </div>
                  <div>
                    ...
                    <p className="text-[11px] text-slate-600 mt-0.5">
                      Total Belanja: <strong>Rp {(loyaltyStats.totalSpent || 0).toLocaleString()}</strong>
                    </p>
                  </div>
                </div>
              ...
              <div className="p-4 bg-blue-50/70 border border-blue-100 rounded-2xl space-y-3 mt-3">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] font-black text-blue-900 uppercase tracking-wider flex items-center gap-1.5">
                    <Wrench className="w-3.5 h-3.5 text-blue-600" /> Teknisi / Mekanik yang Mengerjakan
                  </label>
                  <span className="text-[9px] font-bold bg-white text-blue-600 px-2 py-0.5 rounded border border-blue-100 uppercase">
                    Dicatat Kasir/PIC
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-500 uppercase">Pilih Mekanik</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-500 uppercase">Persentase Bonus Transaksi (%)</label>
                    <div className="relative">
                      <input 
                        ...
                        onChange={e => setFormData(prev => ({ ...prev, mechanicBonusPercent: Number(e.target.value) }))}
                  ...
                  <div key={p.partId} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-xl">
                    <div className="flex-1 min-w-0 pr-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        ...
                        {partObj?.rackCode && (
                          <span className="text-[10px] font-bold text-amber-800 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200/60 flex items-center gap-1">
                    ...
                    {formData.type === 'Retail' && <div className="mt-2">
                      {p.wholesaleType ? <div className="flex justify-between rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-800"><span>Harga grosir: Rp {(p.wholesaleUnitPrice || p.priceAtTime).toLocaleString()} / unit</span><button type="button" onClick={() => clearItemWholesale(p.partId)} className="text-rose-600">Reset</button></div> : <button type="button" onClick={() => { setWholesalePartId(p.partId); setItemWholesaleValue(''); }} className="mt-3 rounded-lg border border-amber-200 bg-white px-2 py-1.5 text-[10px] font-black text-amber-700 transition-colors hover:bg-amber-100 hover:text-amber-900 active:scale-[0.98]">+ Atur harga grosir produk ini</button>}
                    ...
                    {formData.type === 'Retail' && <div className="mt-3 border-t border-slate-100 pt-2"><button type="button" onClick={() => setWarrantyPartId(warrantyPartId === p.partId ? null : p.partId)} className="text-[10px] font-bold text-blue-700 hover:text-blue-900 hover:underline">{warrantyPartId === p.partId ? 'Tutup pengaturan garansi' : '+ Atur garansi produk'}</button>{warrantyPartId === p.partId && <div className="mt-2 grid grid-cols-1 gap-2 rounded-xl border border-blue-200 bg-blue-50 p-2"><label className="text-[9px] font-bold text-slate-600">Durasi garansi (hari)<input type="number" min="0" value={p.warrantyDurationDays || 0} onChange={e => updatePartWarranty(p.partId, { warrantyDurationDays: Number(e.target.value) })} className="mt-1 h-8 w-full rounded-lg border bg-white px-2 text-[10px]" /></label><label className="text-[9px] font-bold text-slate-600">Ketentuan<textarea value={p.warrantyTerms || ''} onChange={e => updatePartWarranty(p.partId, { warrantyTerms: e.target.value })} className="mt-1 min-h-12 w-full rounded-lg border bg-white px-2 py-1 text-[10px]" /></label></div>}</div>}
                    <div className="mt-4 flex items-center gap-1"><button type="button" disabled={p.quantity <= 1} onClick={() => changePartQuantity(p.partId, p.quantity - 1)} className="h-7 w-7 rounded-lg border text-sm font-black disabled:opacity-30">−</button><span className="min-w-8 text-center text-xs font-black">{p.quantity}</span><button type="button" disabled={p.quantity >= (parts.find(item => item.id === p.partId)?.stock || 0)} onClick={() => changePartQuantity(p.partId, p.quantity + 1)} className="h-7 w-7 rounded-lg border text-sm font-black disabled:opacity-30">+</button></div>
        ...
        <div className="pt-4 space-y-4 bg-slate-50 -mx-6 px-6 py-6 border-t border-slate-100">
          <div className="space-y-2 pb-2 border-b border-slate-200/60">
            ...
            {discountAmount > 0 && (
              <div className="flex justify-between items-center text-xs">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-black text-emerald-600">- Rp {discountAmount.toLocaleString()}</span>
                  <button 
                    type="button" 
          ...
          <div className="flex justify-between items-center">
             <div>
               ...
               {discountAmount > 0 && (
                 <span className="text-[10px] font-bold text-emerald-600">Hemat Rp {discountAmount.toLocaleString()}</span>
               )}
             </div>
             <span className="text-2xl font-black text-blue-600">Rp {(grandTotal || 0).toLocaleString()}</span>
          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-1">
                            <span className="text-[10px] text-slate-500 font-bold uppercase">
                              {part.category} • Stok: <strong className={part.stock <= part.minStock ? "text-rose-600" : "text-emerald-600"}>{part.stock} pcs</strong>
                            </span>

                            {/* Location Badge */}
                            <span className="text-[10px] font-bold text-amber-800 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200/80 flex items-center gap-1">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <span className="text-[9px] font-black text-blue-600 group-hover:underline mt-1 block">
                            + Tambah
                          </span>
                        </div>
                      </button>
                      ...
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isFrequent && (
                              <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-amber-500 text-white flex items-center gap-0.5">

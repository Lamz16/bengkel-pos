              ...
              {usedParts.map(p => {
                ...
                return (
                  <div key={p.partId} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-xl">
                    ...
                    {formData.type === 'Retail' && <div className="mt-4 mb-4 border-t border-slate-100 pt-3">
                      ...
                      {wholesalePartId === p.partId && <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50 p-2 space-y-2"><div className="grid grid-cols-3 gap-1.5"><button type="button" onClick={() => setItemWholesaleType('percent')} className={`h-8 rounded-lg text-[10px] font-black transition-colors ${itemWholesaleType === 'percent' ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-slate-600 hover:bg-amber-100 hover:text-amber-900'}`}>%</button><button type="button" onClick={() => setItemWholesaleType('nominal')} className={`h-8 rounded-lg text-[10px] font-black transition-colors ${itemWholesaleType === 'nominal' ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-slate-600 hover:bg-amber-100 hover:text-amber-900'}`}>Pot. Rp</button><button type="button" onClick={() => setItemWholesaleType('unit_price')} className={`h-8 rounded-lg text-[10px] font-black transition-colors ${itemWholesaleType === 'unit_price' ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-slate-600 hover:bg-amber-100 hover:text-amber-900'}`}>Harga Rp</button></div><div className="flex gap-1"><input type="number" min="0" value={itemWholesaleValue} onChange={e => setItemWholesaleValue(e.target.value)} placeholder={itemWholesaleType === 'percent' ? '10' : '50000'} className="h-8 min-w-0 flex-1 rounded-lg border px-2 text-[10px]" /><button type="button" onClick={() => applyItemWholesale(p.partId)} className="h-8 rounded-lg bg-amber-600 px-2 text-[10px] font-bold text-white">Terapkan</button><button type="button" onClick={() => setWholesalePartId(null)} className="h-8 rounded-lg border px-2 text-[10px]">Batal</button></div><p className="text-[9px] text-slate-500">Harga normal: Rp {(p.normalPriceAtTime || p.priceAtTime).toLocaleString()} / unit</p></div>}
                    </div>}
                                        <div className="mt-2 flex items-center gap-1"><button type="button" disabled={p.quantity <= 1} onClick={() => changePartQuantity(p.partId, p.quantity - 1)} className="h-7 w-7 rounded-lg border text-sm font-black disabled:opacity-30">−</button><span className="min-w-8 text-center text-xs font-black">{p.quantity}</span><button type="button" disabled={p.quantity >= (parts.find(item => item.id === p.partId)?.stock || 0)} onClick={() => changePartQuantity(p.partId, p.quantity + 1)} className="h-7 w-7 rounded-lg border text-sm font-black disabled:opacity-30">+</button></div>

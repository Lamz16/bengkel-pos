...
import { CheckCircle2, ChevronLeft, ChevronRight, Copy, RefreshCw, XCircle } from 'lucide-react';
...
export const ActivityLogView = () => {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-x-auto bg-white border border-slate-100 rounded-3xl">
      <table className="w-full min-w-[950px] text-left"><thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="p-4">Waktu</th><th className="p-4">Role / pengguna</th><th className="p-4">Tindakan</th><th className="p-4">Status</th><th className="p-4">Keterangan</th><th className="p-4">Teknis</th></tr></thead>
        <tbody>{rows.map(row => <tr key={row.id} className="border-t border-slate-100 text-xs"><td className="p-4 whitespace-nowrap text-slate-500">{new Date(row.createdAt).toLocaleString('id-ID')}</td><td className="p-4"><p className="font-bold text-slate-800">{row.userName || 'Sistem'}</p><p className="text-[10px] text-slate-400">{row.role || '-'}</p></td><td className="p-4 font-mono text-slate-600">{row.action}</td><td className="p-4"><span className={row.success ? 'inline-flex gap-1 items-center text-emerald-700 bg-emerald-50 px-2 py-1 rounded-lg font-bold' : 'inline-flex gap-1 items-center text-rose-700 bg-rose-50 px-2 py-1 rounded-lg font-bold'}>{row.success ? <CheckCircle2 className="w-3.5 h-3.5"/> : <XCircle className="w-3.5 h-3.5"/>}{row.statusCode}</span></td><td className="p-4 text-slate-600 max-w-[320px]">{row.description}</td><td className="p-4"><button disabled={!row.technicalDetail} onClick={() => row.technicalDetail && navigator.clipboard.writeText(row.technicalDetail)} className="inline-flex items-center gap-1 px-2 py-1 border rounded-lg text-slate-600 disabled:opacity-40"><Copy className="w-3.5 h-3.5"/> Salin</button></td></tr>)}
          {!loading && rows.length === 0 && <tr><td colSpan={6} className="p-12 text-center text-slate-400">Belum ada log aktivitas.</td></tr>}</tbody></table>
    </div>

...
export const ServiceDetail: React.FC<ServiceDetailProps> = ({ 
  ...
  const dueDays = service.paymentDueDate ? Math.floor((new Date(service.paymentDueDate).setHours(0, 0, 0, 0) - new Date().setHours(0, 0, 0, 0)) / 86_400_000) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={cn('p-4 rounded-2xl border space-y-3', service.paymentStatus === 'Paid' ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200')}>
        ...
        {remainingAmount > 0 && <div className="border-t border-amber-200/70 pt-3 grid grid-cols-2 gap-2"><CurrencyInput value={paymentAmount} onValueChange={value => setPaymentAmount(String(value))} placeholder="Nominal bayar" className="h-10 rounded-xl border border-amber-200 bg-white px-3 text-xs font-bold"/><select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value as ServicePaymentMethod)} className="h-10 rounded-xl border border-amber-200 bg-white px-3 text-xs font-bold"><option value="Cash">Cash</option><option value="Transfer">Transfer</option><option value="QRIS">QRIS</option></select>{paymentMethod !== 'Cash' && <input value={paymentReference} onChange={event => setPaymentReference(event.target.value)} placeholder="No. referensi (opsional)" className="col-span-2 h-10 rounded-xl border border-amber-200 bg-white px-3 text-xs font-bold"/>}<button type="button" disabled={isSavingPayment || !Number(paymentAmount)} onClick={async () => { setIsSavingPayment(true); try { await onAddPayment(service.id, { amount: Number(paymentAmount), paymentMethod, referenceNo: paymentReference || undefined }); setPaymentAmount(''); setPaymentReference(''); } catch (error: any) { alert(error.message || 'Pembayaran gagal dicatat.'); } finally { setIsSavingPayment(false); } }} className="col-span-2 h-11 rounded-xl bg-emerald-600 text-xs font-black text-white disabled:opacity-60">{isSavingPayment ? 'Menyimpan...' : 'Catat Pembayaran'}</button></div>}
      </div>

      {remainingAmount > 0 && dueDays !== null && <div className={cn('rounded-2xl border p-3 text-xs font-bold flex items-center gap-2', dueDays < 0 ? 'border-rose-200 bg-rose-50 text-rose-800' : dueDays === 0 ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-blue-100 bg-blue-50 text-blue-800')}><AlertTriangle className="w-4 h-4 shrink-0"/>{dueDays < 0 ? `Piutang terlambat ${Math.abs(dueDays)} hari.` : dueDays === 0 ? 'Piutang jatuh tempo hari ini.' : `Piutang jatuh tempo dalam ${dueDays} hari.`} Sisa: Rp {remainingAmount.toLocaleString('id-ID')}.</div>}

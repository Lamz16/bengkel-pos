...
export class ServiceController {
  async getServices(req: Request, res: Response) {
    try {
      ...
      res.json(await workshopService.getServices());
    } catch (err: any) { res.status(500).json({ error: err.message || 'Gagal memuat daftar servis' }); }
  }
  ...
  async updateStatus(req: Request, res: Response) {
    try {
      ...
      if (!status) return res.status(400).json({ error: 'Status pengerjaan servis wajib diisi.' });
      const expVer = expectedVersion !== undefined ? Number(expectedVersion) : (version !== undefined ? Number(version) : undefined);
  ...
  async claimWarranty(req: Request, res: Response) {
    try {
      ...
      if (!reason) return res.status(400).json({ error: 'Alasan klaim garansi wajib diisi.' });
      const result = await workshopService.applyWarrantyClaim({ serviceId: req.params.id, reason, isAbsentNextDay: !!isAbsentNextDay });
      res.json({ success: true, ...result });
    } catch (err: any) { res.status(500).json({ error: err.message || 'Gagal memproses klaim garansi' }); }
  }

  async processReturn(req: Request, res: Response) {
    try { res.json(await workshopService.processReturn({ serviceId: req.params.id, reason: req.body.reason, items: req.body.items || [] })); }
    catch (err: any) {

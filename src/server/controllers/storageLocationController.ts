...
const asLocation = (location: any) => ({
  ...
  slotCode: location.slotCode || undefined, positionNote: location.positionNote || undefined,
...
export class StorageLocationController {
      ...
      const result = await (prisma as any).$transaction(async (tx: any) => {
        ...
        for (let levelIndex = 0; levelIndex < levels; levelIndex += 1) {
          ...
          for (let slotIndex = 0; slotIndex < slots; slotIndex += 1) {
            ...
            locations.push(await tx.storageLocation.create({ data: { zoneId, type: 'RACK_SLOT', code: `${rack.code}-${level.code}-${slotCode}`, name: `${rack.code} Tingkat ${level.code} Slot ${slotCode}`, rackId: rack.id, levelId: level.id, slotCode } }));
          }
        }
  ...
  createNonRack = async (req: Request, res: Response) => {
    const { zoneId, type, code, name, positionNote } = req.body;
    ...
    try {
      if (!isDbConnected()) {
        const item = { id: `loc-${Date.now()}`, zoneId, type, code: code.trim().toUpperCase(), name: name?.trim() || code.trim().toUpperCase(), positionNote, isActive: true, stockCount: 0 };
        (memoryStore as any).storageLocations = [...((memoryStore as any).storageLocations || []), item];
        return res.status(201).json(item);
      }
      const item = await (prisma as any).storageLocation.create({ data: { zoneId, type, code: code.trim().toUpperCase(), name: name?.trim() || code.trim().toUpperCase(), positionNote: positionNote || null } });
